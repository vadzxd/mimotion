...
function persist_execute_log {
  ...
  current_cron=$(< .github/workflows/run.yml grep '# randomized'|awk '{print substr($0, index($0,$3))}')
  {
    echo "current cron:"
  ...
  "${sed_prefix[@]}" -E "s/(- cron: ')[0-9]+( [^[:space:]]+ \* \* \*')([[:space:]]*# randomized)/\1$((RANDOM % 59)) ${cron_hours} * * *'\3/g" .github/workflows/run.yml
  current_cron=$(< .github/workflows/run.yml grep '# randomized'|awk '{print substr($0, index($0,$3))}')
  {
    echo "next cron:"
    convert_utc_to_shanghai "$current_cron"
